...
def main():
    ...
    # 路径配置
    ckpt_path = "checkpoints/iFlame_best.pth"
    my_pc_path = "data/preview_meshes/val/val_preview_009_04379243_72edff728d75b0b8f994e2a430ba61bf_dec05.obj"
    output_dir = "outputs"
    ...
    # ==========================================
    # 5. 准备初始输入 Token (完全对齐训练时的数据分布)
    # ==========================================
    print("🧠 正在准备自回归生成的初始 Token 序列...")
    
    prompt_obj_path = my_pc_path  # 直接使用同一文件作为 Prompt 输入，确保数据分布完全一致 
    prompt_ratio = 0.30  
    ...
    # 【新增】初始化变量，扩大作用域，以便后续对比使用
    full_seq = None
    prefix_length = 0

    try:
        print(f"🧩 正在从 {prompt_obj_path} 提取初始 Token...")
        prompt_mesh = trimesh.load(prompt_obj_path, force='mesh', process=False)
        v = prompt_mesh.vertices.copy()
        prompt_mesh.vertices = normalize_vertices(v)
        ...
        print(f"📊 该 Prompt 模型的总 Token 长度为: {total_len}")
        ...
        print(f"✂️ 设定保留比例为 {prompt_ratio*100:.1f}%，实际截取了前 {prefix_length} 个 Token。")
    ...
    try:
        ...
        # 解码为 trimesh 对象
        mesh_recon, recon_tokens = tokenizer.decode_mesh(p_seq, b_seq, o_seq)

        recon_tokens = np.array(recon_tokens)
        raw_tokens = np.array(raw_tokens)  # 使用之前提取的完整 Token 进行对比
        
        print(f"📊 解码完成！重构 Token 长度: {len(recon_tokens)}, 原始 Token 长度: {len(raw_tokens)}")
