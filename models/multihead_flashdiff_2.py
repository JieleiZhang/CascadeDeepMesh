...
# 【修改点 1】：注释掉 flash_attn，V100 无法使用这个库
...
class MultiheadFlashlinearrope(nn.Module):
    ...
    def forward(self, x, context=None, causal=False):
        ...
        if self.kv_cache_enabled: 
            ...
            q = q.transpose(1, 2) # [B, num_heads, q_len, head_dim]
            k = k.transpose(1, 2) # [B, num_heads, kv_len, head_dim]
            v = v.transpose(1, 2) # [B, num_heads, kv_len, head_dim]   
            
            # 【🔥🔥🔥 核心修复区：区分自注意力与交叉注意力 🔥🔥🔥】
            if context is not None:
                # 交叉注意力模式：长度不匹配，使用 PyTorch 官方 SDPA 防崩溃
                # 注意：交叉注意力通常不需要传入 causal mask
                attn_output = F.scaled_dot_product_attention(
                    q, k, v, is_causal=False
                )
            else:
                # 自注意力模式：长度匹配，安全使用 lightning_attn 加速
                attn_output = lightning_attn_func(q, k, v, self.slope_rate)

        # 整理维度返回 [B, Seq_len, Embed_dim]
        # 注意：无论是 SDPA 还是 lightning_attn，输出维度都是 [B, num_heads, seq_len, head_dim]
